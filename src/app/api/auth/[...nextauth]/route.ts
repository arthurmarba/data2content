// src/app/api/auth/[...nextauth]/route.ts (v6.14 - Usa Redirect para Sinalizar Seleção Pendente)
// - Modifica callback 'redirect' para adicionar ?action=selectIgAccount se necessário.
...
// Interface atualizada para incluir token opcional
interface RedirectCallback { baseUrl: string; token?: JWT | null; }
  ...
  callbacks: {
    ...
    async jwt({ token, user, account, profile, trigger }: JwtCallback): Promise<JWT> {
        ...
        if (isSignInOrSignUp && account) {
            if (account.provider === 'facebook') { /* ... (lógica FB mantida, incluindo chamada a fetchAvailableInstagramAccounts e storeTemporaryLlat) ... */
    ...
    // --- Callback redirect ATUALIZADO COM LOGS ---
    async redirect({ baseUrl, token }: RedirectCallback): Promise<string> {
        const TAG_REDIRECT = '[NextAuth Redirect Callback]'; // Tag para logs
        const defaultRedirectUrl = `${baseUrl}/dashboard`;

        logger.debug(`${TAG_REDIRECT} Iniciado. BaseURL: ${baseUrl}`);
        // Loga o token recebido (ou parte dele) para verificar seu conteúdo
        // Cuidado ao logar o token inteiro em produção. Logando apenas ID e pending status.
        logger.debug(`${TAG_REDIRECT} Token recebido: ${token ? `ID=${token.id}, Pending=${token.pendingInstagramConnection}` : 'NULL'}`);

        // Verifica se o token final indica que a seleção IG está pendente
        if (token?.pendingInstagramConnection) {
            const redirectUrl = `${defaultRedirectUrl}?action=selectIgAccount`;
            logger.info(`${TAG_REDIRECT} Conexão IG pendente detectada. Redirecionando para: ${redirectUrl}`);
            return redirectUrl; // Adiciona o parâmetro
        }

        // Caso contrário, redireciona para o dashboard padrão
        logger.info(`${TAG_REDIRECT} Nenhuma conexão pendente. Redirecionando para: ${defaultRedirectUrl}`);
