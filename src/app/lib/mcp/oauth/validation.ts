...
export function parseOAuthClientScopes(value: unknown): string[] {
  const supported = new Set(getMcpOAuthSupportedScopes());
  const requested = typeof value === "string"
    ? value.split(/\s+/).map((scope) => scope.trim()).filter(Boolean)
    : [];
  const scopes = requested.length > 0 ? [...new Set(requested)] : getMcpSupportedScopes();
  if (scopes.some((scope) => !supported.has(scope))) {
    throw new McpOAuthError("invalid_scope", 400, "Um ou mais scopes solicitados não são suportados.");
  }
  return scopes;
...
export function parseOAuthScopesForResource(
  value: unknown,
  resource: string,
  useDefault = true,
): string[] {
  const admin = isMcpAdminResource(resource);
  const supported = new Set(admin ? getMcpAdminSupportedScopes() : getMcpSupportedScopes());
  const requested = typeof value === "string"
    ? value.split(/\s+/).map((scope) => scope.trim()).filter(Boolean)
    : [];
  const scopes = requested.length > 0 ? [...new Set(requested)] : useDefault ? [...supported] : [];
  if (scopes.some((scope) => !supported.has(scope))) {
    throw new McpOAuthError("invalid_scope", 400, "Um ou mais scopes não pertencem ao recurso MCP solicitado.");
  }
  const requiredScope = admin ? getMcpAdminRequiredScope() : getMcpRequiredScope();
  if (!scopes.includes(requiredScope)) {
    throw new McpOAuthError("invalid_scope", 400, `O scope ${requiredScope} é obrigatório.`);
  }
  return scopes;
...
export function assertMcpResource(received: unknown): string {
  if (typeof received !== "string") {
    throw new McpOAuthError("invalid_target", 400, "resource MCP ausente.");
  }
  const allowed = [getMcpServerUrl(), getMcpAdminServerUrl()];
  if (!allowed.includes(received)) {
    throw new McpOAuthError("invalid_target", 400, "resource diferente dos MCPs Data2Content autorizados.");
  }
  return received;
}
