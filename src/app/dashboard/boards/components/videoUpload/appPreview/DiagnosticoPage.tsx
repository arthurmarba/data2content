...
export function MapaCard({
  ...
  const cardShell: React.CSSProperties = noShell
    ? {
        padding: "0",
  ...
  // ── Propósito (Meu Norte) — convite calmo quando o criador ainda não declarou ──
  // O propósito é a base do mapa (etapa 1–2 da jornada). Assinantes antigos que
  // entraram na nova experiência sem passar pelo onboarding não têm propósito;
  // este convite os leva a declará-lo, sem overlay forçado. Aparece só quando
  // falta propósito E há como abrir o "Meu Norte".
  const PurposePrompt = (!hasPurpose && onOpenNorte) ? (
    <button
      type="button"
      onClick={onOpenNorte}
      style={{
    ...
    >
      <span style={{
        ...
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path d="M12 2l2.5 7.5H22l-6 4.5 2.3 7.5L12 17l-6.3 4.5L8 14 2 9.5h7.5L12 2z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round"/>
        </svg>
      </span>
      <span style={{ flex: 1, minWidth: 0 }}>
        <span style={{ display: "block", fontSize: 13, fontWeight: 600, color: TEXT_PRIMARY_HEX, letterSpacing: -0.1 }}>
          Defina seu norte
        </span>
        <span style={{ display: "block", fontSize: 12, color: TEXT_SECONDARY_HEX, marginTop: 1 }}>
  ...
  if (!hasAnything && hasReadings) {
    return (
      <div style={noShell ? { ...cardShell, padding: "0" } : { ...cardShell, padding: "22px 22px 6px" }}>
        {MapaHeader}
        <p style={{ fontSize: 16, fontWeight: 600, color: TEXT_BODY_HEX, margin: 0, lineHeight: 1.45 }}>
          Sinais em análise.
        </p>
...
interface Props {
  ...
  onRetryGenerateIdeas?: () => void;
  /** Fase 1 — o card "Sua Audiência" pede pautas semeadas por um território
   *  (Etapa 3 → Etapa 9). O shell dispara a geração e leva o criador às pautas. */
  ...
  onOpenSurvey?: () => void;
  /** Abre "Meu Norte" para declarar/editar o propósito (usado pelo prompt do card). */
  onOpenNorte?: () => void;
