...
export default function PublicSharePage({ params }: { params: { token: string } }) {
    ...
    const sanitizedSnapshots = useMemo(() => {
        ...
        const raw = Array.isArray(metric.dailySnapshots) ? metric.dailySnapshots : [];
        const normalized = raw
            .map((item) => {
                const date = item?.date ? new Date(item.date) : null;
                if (!date || Number.isNaN(date.getTime())) return null;

                const dailyViews = (item as any).dailyViews ?? (item as any).dailyReach ?? (item as any).dailyImpressions ?? 0;
                const dailyLikes = (item as any).dailyLikes ?? 0;
            ...
            .filter((item): item is { date: string; dailyViews: number; dailyLikes: number } => Boolean(item));
                        ...
                        <div className={`${glassClass} p-6 h-[400px] flex flex-col w-full`}>
                            ...
                            {hasHistoricalSeries ? (
                                ...
                                <div className="flex-1 flex flex-col items-center justify-center text-gray-500 bg-gray-50/50 rounded-xl border border-dashed border-gray-200 text-center px-6">
                                    ...
                                    <p className="text-xs text-gray-400">Coletamos pelo menos 2 dias para exibir o gráfico. Ainda assim, veja o último registro:</p>
                                    {chartData[0] && (
                                        <div className="mt-3 text-xs bg-white rounded-lg border border-gray-200 px-3 py-2 shadow-sm w-full max-w-xs">
