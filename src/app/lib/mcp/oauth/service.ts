...
export async function createMcpConsentRequest(
  ...
  const resource = assertExactResource(input.resource, getMcpServerUrl());
  ...
  if (!isScopeSubset(scopes, client.scope)) {
    throw new McpOAuthError("invalid_scope", 400, "O cliente não pode solicitar um ou mais scopes.");
  }
...
export async function approveMcpConsent(token: string, sessionUserId: string): Promise<string> {
  ...
  const entitlement = await getMcpEntitlement(sessionUserId);
  if (!entitlement.eligible) {
    throw new McpOAuthError("subscription_required", 403, "A assinatura não está ativa.");
  }
...
export async function exchangeMcpAuthorizationCode(form: URLSearchParams) {
  ...
  const resource = assertExactResource(form.get("resource"), getMcpServerUrl());
  await connectToDatabase();
  ...
  if (!entitlement.eligible) {
    throw new McpOAuthError("invalid_grant", 400, "A assinatura vinculada não está ativa.");
  }
...
export async function exchangeMcpRefreshToken(form: URLSearchParams) {
  ...
  const scopes = form.has("scope") ? parseOAuthScopes(form.get("scope")) : existing.scope;
