...
async function POST(request: Request) { return executeVideoAnalysis(request, await getServerSession()); }
...
describe("MM88 runbook", () => {
  it("documenta checklist E2E, rollback e verificação de cleanup sem secrets", () => {
    const runbookPath = path.join(
      process.cwd(),
      "docs/video-narrativa/MM88_GATED_REAL_ENDPOINT_E2E_BETA_RUNBOOK.md",
    );
