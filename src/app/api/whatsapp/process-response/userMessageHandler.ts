...
// Versão: vShortTermMemory_CtxExtract_13_FIXED_WamidHandling+IGCTA
// - ATUALIZADO: Anexa PS orientando a conectar o Instagram quando payload.igConnected === false.
...
const HANDLER_TAG_BASE = '[UserMsgHandler vShortTermMemory_CtxExtract_13_FIXED_WamidHandling+IGCTA]';

/** Helper: anexa PS de conexão do Instagram quando necessário */
function withIgCta(text: string, igConnected?: boolean): string {
  if (igConnected) return text;

  const lower = text.toLowerCase();
  // Evita duplicar CTA se a resposta já falar de conectar Instagram
  const alreadyMentions =
    lower.includes('conectar instagram') ||
    lower.includes('conecte seu instagram') ||
    lower.includes('conecte o instagram') ||
    (lower.includes('perfil') && lower.includes('conectar instagram'));

  if (alreadyMentions) return text;

  return (
    text +
    '\n\n🔗 *Conecte seu Instagram*: para eu analisar seus conteúdos e responder com dados reais, vá na plataforma e acesse *Perfil → Conectar Instagram*.'
  );
}
...
`;
  ...
  try {
    ...
    const modelForExtraction =
      (typeof CONTEXT_EXTRACTION_MODEL !== 'undefined' ? CONTEXT_EXTRACTION_MODEL : process.env.CONTEXT_EXTRACTION_MODEL) ||
      'gpt-3.5-turbo';
    const tempForExtraction =
      (typeof CONTEXT_EXTRACTION_TEMP !== 'undefined' ? CONTEXT_EXTRACTION_TEMP : Number(process.env.CONTEXT_EXTRACTION_TEMP)) ??
      0.2;
    const maxTokensForExtraction =
      (typeof CONTEXT_EXTRACTION_MAX_TOKENS !== 'undefined'
        ? CONTEXT_EXTRACTION_MAX_TOKENS
        : Number(process.env.CONTEXT_EXTRACTION_MAX_TOKENS)) || 100;
    ...
    if (!jsonMatch || !jsonMatch[0]) {
      logger.warn(
        `${TAG} Nenhum JSON encontrado na resposta da extração de tópico/entidades. Resposta: ${extractionResultText}`
      );
    ...
    const context: ILastResponseContext = {
      ...
      entities:
        parsedJson && Array.isArray(parsedJson.entities)
          ? parsedJson.entities.map((e: any) => String(e).trim()).filter((e: string) => e)
          : [],
      timestamp: Date.now(),
    ...
    if (!context.topic && (!context.entities || context.entities.length === 0) && !context.wasQuestion) {
      logger.debug(
        `${TAG} Extração de contexto não produziu tópico, entidades ou indicativo de pergunta. Retornando null após tentativa de parse.`
      );
      if (!context.wasQuestion) return null;
    }

    logger.info(
      `${TAG} Contexto extraído da resposta da IA (FINAL) - Topic: "${
        context.topic ? context.topic.substring(0, 50) + '...' : 'N/A'
      }", Entities: [${context.entities?.join(', ')}], Timestamp: ${context.timestamp}, WasQuestion: ${context.wasQuestion}`
    );
    return context;
  } catch (error) {
...
async function generateInstigatingQuestion(
  ...
  if (potentialLastSegment.trim().endsWith('?')) {
    logger.debug(
      `${TAG} Resposta da IA (ou seu último segmento) já termina com uma pergunta. Pulando geração de pergunta instigante.`
    );
    return null;
  }
...
`;
  ...
  try {
    ...
    const model =
      (typeof INSTIGATING_QUESTION_MODEL !== 'undefined' ? INSTIGATING_QUESTION_MODEL : process.env.INSTIGATING_QUESTION_MODEL) ||
      'gpt-3.5-turbo';
    const temperature =
      (typeof INSTIGATING_QUESTION_TEMP !== 'undefined' ? INSTIGATING_QUESTION_TEMP : Number(process.env.INSTIGATING_QUESTION_TEMP)) ??
      0.7;
    const max_tokens =
      (typeof INSTIGATING_QUESTION_MAX_TOKENS !== 'undefined'
        ? INSTIGATING_QUESTION_MAX_TOKENS
        : Number(process.env.INSTIGATING_QUESTION_MAX_TOKENS)) || 80;
...
export async function handleUserMessage(payload: ProcessRequestBody): Promise<NextResponse> {
  ...
  if (!fromPhone || !incomingText) {
    ...
    return NextResponse.json(
      { error: 'Invalid payload for user message: fromPhone or incomingText missing.' },
      { status: 400 }
    );
  }
  ...
  let initialDataLoadStartTime = 0;
  let updateState1StartTime = 0;
  let getDialogueState2StartTime = 0;
  let determineIntentStartTimeGlobal = 0;
  let updateState2StartTime = 0;
  let getDialogueState3StartTime = 0;
  let getDSCountersCallStartTimeGlobal = 0;
  let getDSInterruptStartTimeGlobal = 0;
  let getDSConfirmTaskStartTime = 0;
  let getDSClearPendingStartTime = 0;
  let getDSClearPending2StartTime = 0;
  let getDSFinalSaveStartTime = 0;
  ...
  try {
    ...
    logger.debug(
      `${handlerTAG} Carregamento inicial (lookupUserById, getDialogueState, getConversationHistory) levou ${
        Date.now() - initialDataLoadStartTime
      }ms.`
    );
    ...
    logger.debug(
      `${handlerTAG} Dados carregados. Nome: ${firstName}, Histórico: ${historyMessages.length} msgs. Sumário Conv: ${
        dialogueState.conversationSummary ? '"' + extractExcerpt(dialogueState.conversationSummary, 30) + '"' : 'N/A'
      }`
    );
    ...
    if (!isActiveLike(user.planStatus)) {
      try {
        const wamid = await sendWhatsAppMessage(
          fromPhone,
          `Olá ${firstName}! Seu plano está ${user.planStatus}. Para continuar usando o Mobi, reative seu plano em nosso site.`
        );
    ...
    logger.debug(
      `${handlerTAG} stateService.updateDialogueState (início processamento) levou ${Date.now() - updateState1StartTime}ms.`
    );

    getDialogueState2StartTime = Date.now();
    dialogueState = await stateService.getDialogueState(userId);
    logger.debug(
      `${handlerTAG} stateService.getDialogueState (após update início) levou ${
        Date.now() - getDialogueState2StartTime
      }ms.`
    );
    logger.info(
      `${handlerTAG} Estado de processamento atualizado. currentProcessingMessageId setado para ${messageId_MsgAtual}. QueryExcerpt: "${queryExcerpt_MsgAtual}"`
    );
  } catch (err) {
    ...
    try {
      const wamid = await sendWhatsAppMessage(
        fromPhone,
        "Desculpe, tive um problema ao iniciar o processamento da sua mensagem. Tente novamente em instantes."
      );
      logger.info(
        `${handlerTAG} Mensagem de erro (carregamento inicial) enviada. UserMsgID: ${messageId_MsgAtual}, WhatsAppMsgID: ${wamid}`
      );
    } catch (e: any) {
      logger.error(
        `${handlerTAG} Falha CRÍTICA ao enviar mensagem de erro (carregamento inicial) para UserMsgID: ${messageId_MsgAtual}:`,
        e
      );
    }
    ...
    return NextResponse.json(
      {
        error: `Failed to load initial user data or set processing state: ${
  ...
  if (!normText) {
    ...
    let emptyNormResponse = `${greeting} Não entendi bem o que você disse. Pode repetir, por favor?`;
    // ⬇️ Anexa CTA de IG se aplicável
    emptyNormResponse = withIgCta(emptyNormResponse, payload.igConnected);

    try {
      const wamid = await sendWhatsAppMessage(fromPhone, emptyNormResponse);
      logger.info(
        `${handlerTAG} Mensagem (texto normalizado vazio) enviada. UserMsgID: ${messageId_MsgAtual}, WhatsAppMsgID: ${wamid}`
      );
    } catch (sendError) {
      logger.error(
        `${handlerTAG} FALHA AO ENVIAR mensagem (texto normalizado vazio) para UserMsgID: ${messageId_MsgAtual}. Erro:`,
        sendError
      );
    }
    await stateService.updateDialogueState(userId, {
  ...
  if (!currentDeterminedIntent) {
    ...
    try {
      ...
      if (intentResult.type === 'special_handled') {
        ...
        if (
          intentResult.intent === 'user_confirms_pending_action' ||
          intentResult.intent === 'user_denies_pending_action'
        ) {
          ...
          !COMPLEX_TASK_INTENTS.includes(currentDeterminedIntent) &&
    ...
    if (
      currentDeterminedIntent &&
      (currentDeterminedIntent.startsWith('user_') ||
        COMPLEX_TASK_INTENTS.includes(currentDeterminedIntent) ||
        currentDeterminedIntent === 'humor_script_request')
    ) {
      try {
        ...
        logger.error(
          `${handlerTAG} Erro ao tentar obter detalhes da intenção (vinda do payload '${currentDeterminedIntent}'):`,
          e
        );
      }
    }
  ...
  if (intentResult && intentResult.type === 'intent_determined' && currentDeterminedIntent) {
    ...
    try {
      ...
      logger.error(
        `${handlerTAG} Erro ao persistir memória para intenção '${currentDeterminedIntent}':`,
        memoryError
      );
    }
  }
  ...
  if (Object.keys(dialogueStateUpdateForTaskStart).length > 0) {
    ...
    logger.debug(
      `${handlerTAG} stateService.updateDialogueState (task start) levou ${Date.now() - updateState2StartTime}ms.`
    );

    getDialogueState3StartTime = Date.now();
    dialogueState = await stateService.getDialogueState(userId);
    logger.debug(
      `${handlerTAG} stateService.getDialogueState (após task start) levou ${Date.now() - getDialogueState3StartTime}ms.`
    );
  }

  if (responseTextForSpecialHandled) {
    let fullResponseForSpecial = responseTextForSpecialHandled;
    const instigatingQuestionForSpecial = await generateInstigatingQuestion(
    ...
    // ⬇️ CTA de IG se aplicável
    fullResponseForSpecial = withIgCta(fullResponseForSpecial, payload.igConnected);

    try {
      const wamid = await sendWhatsAppMessage(fromPhone!, fullResponseForSpecial);
      logger.info(
        `${handlerTAG} Mensagem (intenção especial) enviada. UserMsgID: ${messageId_MsgAtual}, WhatsAppMsgID: ${wamid}, Preview: "${fullResponseForSpecial.substring(
          0,
          50
        )}..."`
      );
      ...
      logger.error(
        `${handlerTAG} FALHA AO ENVIAR mensagem (intenção especial) para UserMsgID: ${messageId_MsgAtual}. Erro:`,
        sendError
      );
    }
    ...
    logger.debug(
      `${handlerTAG} stateService.getDialogueState (para contadores) levou ${Date.now() - getDSCountersCallStartTimeGlobal}ms.`
    );
  ...
  if (dialogueState.lastInteraction && nowForAck - dialogueState.lastInteraction < ACK_SKIP_THRESHOLD_MILLISECONDS) {
    if (historyMessages.length > 0 && historyMessages[historyMessages.length - 1]?.role === 'assistant') {
      logger.info(
        `${handlerTAG} Interação MUITO recente com o assistente (${secondsSinceLastInteraction.toFixed(
          0
        )}s atrás). Pulando quebra-gelo completamente.`
      );
      shouldSkipDynamicAckEntirely = true;
    } else {
      logger.info(
        `${handlerTAG} Interação recente, mas a última mensagem não foi do assistente. Quebra-gelo será considerado (para nome/genérico).`
      );
    }
  }
  if (!shouldSkipDynamicAckEntirely) {
    if (lastInteractionTimeForAck !== 0 && nowForAck - lastInteractionTimeForAck < GREETING_THRESHOLD_MILLISECONDS) {
      logger.info(
        `${handlerTAG} Interação recente (${minutesSinceLastInteraction.toFixed(
          1
        )} min). Quebra-gelo será genérico (sem nome).`
      );
      useNameToAck = false;
      ...
      logger.info(
        `${handlerTAG} Tempo suficiente desde a última interação (${minutesSinceLastInteraction.toFixed(
          1
        )} min). Quebra-gelo usará o nome.`
      );
    }
  } else {
    if (shouldSkipDynamicAckEntirely) {
      ...
      logger.debug(`${handlerTAG} Pulando quebra-gelo por regra interna.`);
    }
  }

  const isLightweightIntentForDynamicAck =
    currentDeterminedIntent === 'social_query' || currentDeterminedIntent === 'meta_query_personal';
  const intentsToSkipDynamicAck = ['user_confirms_pending_action', 'user_denies_pending_action', 'greeting', 'generate_proactive_alert'];
  if (
    !shouldSkipDynamicAckEntirely &&
    ...
    try {
      ...
      if (dynamicAckMessage) {
        try {
          ...
          logger.error(
            `${handlerTAG} FALHA AO ENVIAR quebra-gelo dinâmico para UserMsgID: ${messageId_MsgAtual}. Erro:`,
            sendAckError
          );
        }
      }
  ...
  logger.debug(
    `${handlerTAG} stateService.getDialogueState (para interrupção) levou ${Date.now() - getDSInterruptStartTimeGlobal}ms.`
  );

  if (freshDialogueStateForInterrupt.interruptSignalForMessageId === messageId_MsgAtual) {
    logger.info(
      `${handlerTAG} INTERRUPÇÃO DETECTADA para MsgID: ${messageId_MsgAtual} ("${queryExcerpt_MsgAtual}") APÓS ACK. Sinal: ${freshDialogueStateForInterrupt.interruptSignalForMessageId}. Pulando resposta principal da LLM.`
    );
    ...
    return NextResponse.json(
      { success: true, message: 'Processing interrupted by newer user message after acknowledgement phase.' },
      { status: 200 }
    );
  }
  ...
  if (effectiveIntent === 'EXPLAIN_DATA_SOURCE_FOR_ANALYSIS') {
    systemMessagesForContext.push({
      role: 'system',
      content:
        `O usuário está pedindo a origem dos dados de uma análise recente. Revise histórico/summary e o contexto da última resposta (` +
        `${dialogueState.lastResponseContext?.topic || 'N/A'}). Explique de forma clara e direta.`
    });
    ...
    systemMessagesForContext.push({
      role: 'system',
      content:
        `O usuário quer os números/detalhes de métricas mencionadas recentemente. Revise histórico/summary e o contexto da última resposta (` +
        `${dialogueState.lastResponseContext?.topic || 'N/A'}). Forneça os detalhes com precisão.`
    });
    ...
    systemMessagesForContext.push({
      role: 'system',
      content:
        `O usuário quer esclarecimento sobre sua última resposta (Contexto: ${dialogueState.lastResponseContext?.topic || 'N/A'}). ` +
        `Releia sua última mensagem e explique melhor de forma didática.`
    });
    ...
    systemMessagesForContext.push({
      role: 'system',
      content:
        `O usuário quer continuar/aprofundar o tópico recente (Contexto: ${dialogueState.lastResponseContext?.topic || 'N/A'}). ` +
        `Dê seguimento com informações e perguntas relevantes.`
    });
  ...
  if (currentDeterminedIntent === 'user_confirms_pending_action') {
    ...
    if (lastAIQType === 'confirm_fetch_day_stats' && pendingContext?.originalSuggestion) {
      ...
      if (dialogueState.currentTask?.name === 'ask_community_inspiration') {
        ...
        dialogueState = await stateService.getDialogueState(userId);
        logger.debug(
          `${handlerTAG} stateService.getDialogueState (após confirm task) levou ${Date.now() - getDSConfirmTaskStartTime}ms.`
        );
      }
    ...
    const denialResponse = pickRandom([
      'Entendido. Como posso te ajudar então?',
      'Ok. O que você gostaria de fazer a seguir?',
      'Sem problemas. Em que mais posso ser útil?'
    ]);
    ...
    // ⬇️ CTA de IG se aplicável
    finalDenialResponse = withIgCta(finalDenialResponse, payload.igConnected);
    ...
    try {
      ...
      logger.error(
        `${handlerTAG} FALHA AO ENVIAR mensagem (negação de ação pendente) para UserMsgID: ${messageId_MsgAtual}. Erro:`,
        sendError
      );
    }

    const userDenialMsgHist: ChatCompletionUserMessageParam = { role: 'user', content: incomingText! };
    const assistantDenialResponseHist: ChatCompletionAssistantMessageParam = {
      role: 'assistant',
      content: finalDenialResponse
    };
    const updatedHistoryDeny = [...currentTurnHistory.slice(0, -1), userDenialMsgHist, assistantDenialResponseHist].slice(
      -HISTORY_LIMIT
    );
    ...
    logger.info(
      `${handlerTAG} Havia uma pergunta pendente ('${dialogueState.lastAIQuestionType}'), mas o usuário respondeu com outra intenção ('${currentDeterminedIntent}'). Limpando estado pendente.`
    );
    await stateService.clearPendingActionState(userId);
  ...
  try {
    ...
    if (finalText.trim().length === 0 && !finalText.includes('Ops! Tive uma dificuldade técnica')) {
      ...
      finalText =
        'Hum... não consegui gerar uma resposta completa para isso agora. Você poderia tentar reformular sua pergunta ou tentar novamente em alguns instantes?';
    }
  } catch (err: any) {
  ...
  // ⬇️ CTA de IG se aplicável (resposta principal)
  fullResponseToUser = withIgCta(fullResponseToUser, payload.igConnected);

  try {
    const wamid = await sendWhatsAppMessage(fromPhone!, fullResponseToUser);
    logger.info(
      `${handlerTAG} Resposta principal enviada. UserMsgID: ${messageId_MsgAtual}, WhatsAppMsgID: ${wamid}, Preview: "${fullResponseToUser.substring(
        0,
        100
      )}..."`
    );
    ...
    logger.error(
      `${handlerTAG} FALHA CRÍTICA AO ENVIAR resposta principal para UserMsgID: ${messageId_MsgAtual}. Erro: ${sendError.message}`,
      sendError
    );
    await stateService
  ...
  if (fullResponseToUser && !intentsToAvoidPendingActionSuggestion.includes(effectiveIntent)) {
    ...
    if (pendingActionInfo.suggests && pendingActionInfo.actionType) {
      ...
      if (dialogueState.currentTask) {
        finalDialogueStateUpdate.currentTask = {
          ...dialogueState.currentTask,
          currentStep: `aguardando_confirmacao_sobre_${pendingActionInfo.actionType}`
        };
      }
    ...
    if (
      dialogueState.currentTask &&
      (effectiveIntent === 'social_query' || effectiveIntent === 'meta_query_personal')
    ) {
  ...
  logger.debug(
    `${handlerTAG} stateService.getDialogueState (antes do save final) levou ${Date.now() - getDSFinalSaveStartTime}ms.`
  );
  ...
  if (finalDialogueStateBeforeSave.currentProcessingMessageId === messageId_MsgAtual) {
    ...
    logger.warn(
      `${handlerTAG} currentProcessingMessageId no Redis (${finalDialogueStateBeforeSave.currentProcessingMessageId}) não corresponde ao ID desta tarefa (${messageId_MsgAtual}). Não limpando automaticamente aqui.`
    );
  }
  ...
  const assistantMessageForHistory: ChatCompletionAssistantMessageParam = {
    role: 'assistant',
    content: fullResponseToUser
  };
  ...
  if (historyPromise) {
    try {
      ...
      if (
        finalHistoryForSaving.length > 0 &&
        finalHistoryForSaving[finalHistoryForSaving.length - 1]?.role === 'assistant'
      ) {
  ...
  const lastUserMessageToAdjust =
    lastUserMessageIndexInFinalHistory !== -1 ? finalHistoryForSaving[lastUserMessageIndexInFinalHistory] : undefined;
  if (lastUserMessageToAdjust && lastUserMessageToAdjust.content !== effectiveIncomingText) {
    logger.warn(
      `${handlerTAG} Ajustando a última mensagem do usuário no histórico final para corresponder a effectiveIncomingText (segunda checagem).`
    );
  ...
  const lastAssistantMessageToAdjust =
    lastAssistantMessageIndexInFinalHistory !== -1 ? finalHistoryForSaving[lastAssistantMessageIndexInFinalHistory] : undefined;
  if (lastAssistantMessageToAdjust && lastAssistantMessageToAdjust.content !== fullResponseToUser) {
    logger.warn(
      `${handlerTAG} Ajustando a última mensagem do assistente no histórico final para corresponder a fullResponseToUser.`
    );
