...
export const functionSchemas = [
  ...
  {
    name: 'getDayPCOStats',
    description: 'Retorna um RANKING dos melhores horários/combinações (Dia, Proposta, Contexto) ordenados por uma métrica específica. Use para responder "qual o melhor dia e horário para postar?" ou "ranking de horários por compartilhamento". Retorna uma lista já ordenada.',
    parameters: {
      type: 'object',
      properties: {
        metric: {
          type: 'string',
          enum: ['shares', 'saved', 'likes', 'comments', 'reach', 'views', 'total_interactions'],
          default: 'shares',
          description: "A métrica para ordenar o ranking (padrão: shares)."
        },
        limit: {
          type: 'number',
          default: 5,
...
/* 2.4 getDayPCOStats */
const getDayPCOStats: ExecutorFn = async (args: z.infer<typeof ZodSchemas.GetDayPCOStatsArgsSchema>, loggedUser) => {
  const fnTag = '[fn:getDayPCOStats v1.0.0]'; // Major version update due to logic change
  try {
    const metric = args.metric || 'shares';
    const limit = args.limit || 5;

    logger.info(`${fnTag} Executando para usuário ${loggedUser._id}. Metric: ${metric}, Limit: ${limit}`);
      ...
      for (const [proposal, ctxObj] of Object.entries(propObj as any)) {
        for (const [context, val] of Object.entries(ctxObj as any)) {
          ...
          switch (metric) {
            ...
            case 'likes': value = statsObj.avgLikes || 0; break;
            case 'comments': value = statsObj.avgComments || 0; break;
            case 'reach': value = statsObj.avgReach || 0; break;
            case 'views': value = statsObj.avgViews || 0; break;
            case 'total_interactions': value = statsObj.avgTotalInteractions || 0; break;
    ...
    return { error: "Ocorreu um erro inesperado ao gerar o ranking. Por favor, tente novamente.", ranking: [] };
  }
};
