// src/app/LandingPageClient.tsx (Novo)
// Todo o conteúdo interativo da sua landing page foi movido para cá.
...
// Dynamic imports
// CORREÇÃO FINAL: Ajustamos a importação dinâmica para resolver o módulo e
// então selecionar o componente exportado nomeado (ex: mod.IntroSlide).
// Isso resolve o erro de tipo quando os componentes não usam 'export default'.
const LegacyHero = dynamic(() => import("./landing/components/LegacyHero"));

const DynamicIntroSlide = dynamic(() =>
  import("./landing/components/IntroSlide").then((mod) => mod.IntroSlide)
);
const IntroSlide = withViewport(DynamicIntroSlide);

const DynamicFeaturesSlide = dynamic(() =>
  import("./landing/components/FeaturesSlide").then((mod) => mod.FeaturesSlide)
);
const FeaturesSlide = withViewport(DynamicFeaturesSlide);

const DynamicExamplesSlide = dynamic<{ screenshots: { title: string; imageUrl: string; description: string }[] }>(() =>
  // CORREÇÃO: Alterado de mod.default para mod.ExamplesSlide para corresponder à exportação nomeada.
  import("./landing/components/ExamplesSlide").then((mod) => mod.ExamplesSlide)
);
const ExamplesSlide = withViewport(DynamicExamplesSlide);

const DynamicAnimatedSection = dynamic<any>(() =>
  import("./landing/components/AnimatedSection").then((mod) => mod.default)
);
const AnimatedSection = withViewport(DynamicAnimatedSection);
...
const DynamicCallToAction = dynamic(() =>
  import("./landing/components/CallToAction").then((mod) => mod.default)
);
const CallToAction = withViewport(DynamicCallToAction);
