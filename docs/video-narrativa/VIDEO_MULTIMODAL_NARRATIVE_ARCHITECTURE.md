...
MM60 cria o guichê server-side de upload temporário sob o endpoint `/api/dashboard/mobile-strategic-profile/upload-session`. Ele valida sessão, feature flag, consentimento e metadados, retornando uma sessão de upload mock segura. Não existe upload real nem storage real nesta fase, e o snapshot continua sendo a única memória persistente do perfil estratégio.

MM61 aproxima a experiência do fluxo real de vídeo no Perfil Estratégico mobile. A UI do `+ / Analisar vídeo` seleciona um arquivo local apenas para validar metadados (`name`, `type`, `size`) e consentimento contra a upload-session API. Ainda não há upload real: o arquivo não sai do browser, não há thumbnail/player, não há storage, e o snapshot do Perfil continua sendo a memória persistida após a análise mock existente.

MM62 separa o conceito de upload session da decisão de storage provider. A rota passa a usar uma factory server-side com providers `disabled` e `mock`, enquanto R2/S3/GCS/Cloudinary ficam apenas como modos planejados. Storage real continua em fase futura, sem signed URL real e sem SDK externo; o vídeo segue descartável/não persistido e o snapshot continua sendo a única memória persistida do Perfil.

MM63 cria o primeiro caminho server-side para signed upload session curta, mas somente para allowlist/admin-dev e sem integração de upload real no client. Como não há SDK S3/R2 adequado instalado no repo, a assinatura fica atrás de um signer server-side injetável/testável; fora desse adapter, a rota retorna erro seguro. O vídeo continua não persistido pelo app, a UI não faz PUT, o snapshot segue sendo a memória persistida e Gemini continua fase futura.

MM64 é o primeiro passo de upload real controlado no client: quando a upload-session API retorna `signed_upload_session_created` para allowlist/admin-dev e flags reais, o app envia o arquivo diretamente para a signed URL temporária por `PUT`, sem passar bytes pelo app server. `mock_session_created` continua sendo metadata-only e a preview interna segue sem upload-session/direct upload. O snapshot continua sendo a memória persistida do Perfil, não recebe `uploadUrl` nem `objectKey`; cleanup entra como contrato/API seguro e Gemini continua fora do fluxo.

MM65 prepara o motor de IA real sem fechar o ciclo end-to-end. A nova camada server-side define provider interface, config/env, allowlist, prompt builder, parser e mapper para snapshot, mas o fluxo `+` continua usando análise mock. Upload temporário e provider Gemini ainda não estão conectados como caminho padrão. A resposta da IA precisa passar por parser/sanitizer antes de virar snapshot, e o snapshot continua sendo a memória persistida, sem raw response, signed URL, `uploadUrl` ou `objectKey`.

MM66 fecha o primeiro ciclo real controlado de análise de vídeo apenas para allowlist/admin-dev. O client só chama `/api/dashboard/mobile-strategic-profile/analyze-real` depois de upload signed temporário bem-sucedido e flag pública ativa; o servidor ainda exige flags/env/allowlist completas. A resposta do provider passa por parser, sanitizer e mapper antes de persistir snapshot `gemini_real_allowlist`. O snapshot segue sendo a memória do produto, sem vídeo bruto, raw response, signed URL ou `objectKey`, e o upload temporário deve ser descartado via cleanup após a análise.

MM67 adiciona auditoria real de runtime e smoke harness interno para Gemini sem salvar snapshot ou raw response. MM68 conecta o adapter S3/R2-compatible para transformar `objectKey` transitório em bytes server-side para o Gemini, mantendo signed URL fora do prompt, da resposta e do snapshot.

MM69 prova o runtime real em ambiente local allowlist: env audit `ok=true`, Gemini smoke `ok=true`, storage smoke `ok=true` com upload/GetObject/DeleteObject, e real E2E smoke `e2e_real_passed` via chamadas internas controladas. A limitação registrada é que o caminho pelo browser ainda depende do signer real de signed URL no endpoint de upload-session; o teste controlado validou o ciclo de runtime sem liberar para usuários comuns.

MM70 adiciona a camada de beta hardening antes de qualquer rollout: policy server-side por tier, contador persistente de uso real, bloqueio antes de storage/Gemini, mensagens humanas seguras e checklist de produção/rollback. A decisão de produto permanece: o Perfil da D2C é o diagnóstico vivo do creator, mas cada atualização por vídeo real agora passa por custo controlado e beta fechado.

MM71 transforma a pilha em launch candidate de beta fechado: adiciona readiness operacional sem chamadas externas, checklist Vercel/Preview/Production, roteiro de QA mobile e rollback documentado. O produto segue fechado por allowlist/admin-dev e o endpoint mock continua como fallback seguro.

MM72 transforma o launch candidate em operação de beta fechado: adiciona runbook de operador, smoke test Preview/Production, template de feedback para creators, checklist de Vercel envs, rollback, critérios de aprovação/reprovação e monitoramento de custo/uso. Não cria nova arquitetura, não altera fluxo core, não libera público geral e não toca billing, Stripe, Mídia Kit, Comunidade, navegação real, shells, LoginClient ou NextAuth.
