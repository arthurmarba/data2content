...
function PlannerCalendarSkeleton() {
  return (
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
      {[1, 2, 3, 4].map((row) => (
        <div
          key={`planner-calendar-loading-${row}`}
          className={[
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-2">
            {Array.from({ length: 4 }).map((_, index) => (
              <span key={`planner-calendar-loading-${row}-${index}`} className="h-8 rounded-lg bg-slate-100" />
            ))}
          </div>
        </div>
      ))}
...
const ContentPlannerCalendar = dynamic(
  () => import('@/app/mediakit/components/ContentPlannerCalendar').then((mod) => mod.ContentPlannerCalendar),
  {
    ssr: false,
    loading: () => <PlannerCalendarSkeleton />,
  }
);
...
export type PlannerClientPageProps = {
  viewer?: ViewerInfo;
  compactView?: boolean;
...
export function PlannerClientPageSurface({
  viewer,
  compactView = false,
  onNavigateToScripts,
  viewerPending = false,
  ...
  const viewerUserId =
    typeof viewer?.id === 'string' && viewer.id.trim().length > 0
      ? viewer.id.trim()
      : '';
  const isAdminViewer = viewerRoleFromProp === 'admin';
  ...
  const activeUserId = targetUserId ?? viewerUserId;
  const isAuthenticated = activeUserId.length > 0;
  const isPreviewMode = previewMode || (!isAdminViewer && !isAuthenticated && !viewerPending);
  const [calendarReady, setCalendarReady] = useState(!compactView);
  const plannerDataReady = !compactView || calendarReady;
  ...
  const [resolvedPremiumAccess, setResolvedPremiumAccess] = useState(initialHasAccess);
  const [isBillingLoading, setIsBillingLoading] = useState(shouldCheckBilling);
  const plannerQueryUserId = plannerDataReady && !isPreviewMode ? activeUserId : '';
  const plannerQueryTargetUserId =
    plannerDataReady && !isPreviewMode && isActingOnBehalf ? targetUserId : null;
  ...
  const {
    ...
    targetUserId: plannerQueryTargetUserId,
  ...
  const hasAccess = isAdminViewer || initialHasAccess || resolvedPremiumAccess;
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      fallbackTimeoutId = window.setTimeout(() => setCalendarReady(true), 90);
    });
  ...
  useEffect(() => {
    if (!shouldCheckBilling) {
      setResolvedPremiumAccess(initialHasAccess);
    ...
    async function resolvePlannerAccess() {
      try {
        ...
        if (!response.ok || !payload?.ok) {
          throw new Error(payload?.error || 'Falha ao validar acesso ao planejador.');
        }

        if (cancelled) return;
        const accessMeta = getPlanAccessMeta(payload?.status, payload?.cancelAtPeriodEnd);
        setResolvedPremiumAccess(accessMeta.hasPremiumAccess);
      } catch {
        if (cancelled) return;
        setResolvedPremiumAccess(false);
      } finally {
        if (cancelled) return;
        setIsBillingLoading(false);
      }
    ...
    void resolvePlannerAccess();

    return () => {
      cancelled = true;
    };
  }, [initialHasAccess, shouldCheckBilling]);
