...
export function MobileCalculatorWizard({ open, onClose, onSaved, latestCalculation, suggestedReach, journey = false }: MobileCalculatorWizardProps) {
      ...
      <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="mobile-calculator-wizard-title" className="ds-sheet ds-enter-sheet flex max-h-[min(92dvh,740px)] flex-col overflow-hidden">
        ...
        <header className="shrink-0 px-5 pb-3.5 pt-3.5">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex min-w-0 items-start gap-2.5">
              ...
              <div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-500">Etapa {currentStepNumber} de {visibleSteps.length}</p><h2 id="mobile-calculator-wizard-title" className="mt-0.5 font-display text-[1.45rem] font-bold leading-[1.05] tracking-[-0.03em] text-zinc-950">{journey ? ["O que você vai entregar?", "Como a marca vai usar?", "Qual é o contexto?", "Você já tem um valor habitual?", "Sua referência para negociar"][step] : currentStepCopy.title}</h2></div>
            </div>
            {!journey && <DiagnosticoCloseButton onClose={close} ariaLabel="Fechar calculadora" disabled={isSubmitting} edgeAlign />}
          </div>
        ...
        <main className="min-h-0 flex-1 overflow-y-auto px-5 pb-4 pt-1">
          {step === 0 ? <div className="grid gap-4">
            {journey && <p className="j-calc-intro">Monte as entregas desta proposta.</p>}
            <div className="calculator-reach flex items-center justify-between border-b border-zinc-100 pb-3"><p className="text-[11px] font-medium text-zinc-500">Alcance usado no cálculo</p><p className="text-[13px] font-bold text-zinc-950">{reachLabel ? `${reachLabel} pessoas` : "Métricas do perfil"}</p></div>
            <div className="calculator-deliverables divide-y divide-zinc-100 border-y border-zinc-100">{(journey ? ["reels", "post", "stories"] as FormatKey[] : formatOrder).map((key) => <div key={key} className="flex min-h-14 items-center justify-between gap-3 py-2"><div><p className="text-[15px] font-semibold text-zinc-900">{journey ? {reels:"Reels",post:"Post no feed",stories:"Stories"}[key] : formatLabels[key]}</p>{journey && <small className="j-calc-help">{{reels: isUgc ? "Vídeos para a marca" : "Vídeos no seu perfil",post:"Foto ou carrossel",stories:"Cada sequência tem 3 Stories"}[key]}</small>}</div><div className="flex items-center gap-2"><button type="button" aria-label={`Remover ${formatLabels[key]}`} className="grid h-11 w-11 place-items-center rounded-full border border-zinc-200 text-sm text-zinc-600 disabled:opacity-40" onClick={() => updateQuantity(key, -1)} disabled={quantities[key] === 0}>−</button><span className="w-5 text-center text-[14px] font-semibold text-zinc-950" aria-live="polite">{quantities[key]}</span><button type="button" aria-label={`Adicionar ${formatLabels[key]}`} className="grid h-11 w-11 place-items-center rounded-full bg-zinc-950 text-sm text-white" onClick={() => updateQuantity(key, 1)}>+</button></div></div>)}</div>
            {journey && <WizardSection label="Onde será publicado?"><SegmentedChoice value={contentModel} onChange={setContentModel} options={[{value:"publicidade_perfil",label:"No meu perfil"},{value:"ugc_whitelabel",label:"Só para a marca"}]} /><p className="j-calc-help">{isUgc ? "Você produz e entrega. A marca publica (UGC)." : "Inclui criar o conteúdo e publicar no seu perfil."}</p></WizardSection>}
            {journey && hasDeliverables && <p className="j-calc-selection">{buildDeliverablesLabel(quantities)}</p>}
          ...
          {step === 1 ? <div className="grid gap-4">
            ...
            <WizardSection label="Uso pela marca"><SegmentedChoice value={usageRights} onChange={setUsageRights} options={[{ value: "organico", label: journey ? "Sem anúncios" : "Orgânico" }, { value: "midiapaga", label: journey ? "Em anúncios" : "Mídia paga" }, { value: "global", label: journey ? "Uso amplo" : "Uso global" }]} /><div className="mt-4"><div className="mb-2 flex items-center justify-between gap-3"><p className="text-[11px] font-medium text-zinc-600">Duração de uso</p>{usageRights === "organico" ? <span className="text-[10px] text-zinc-400">Só para mídia paga ou global</span> : null}</div><SegmentedChoice value={paidMediaDuration} onChange={setPaidMediaDuration} disabled={usageRights === "organico"} options={[{ value: "7d", label: "7 dias" }, { value: "15d", label: "15 dias" }, { value: "30d", label: "30 dias" }, { value: "90d", label: "90 dias" }]} /></div></WizardSection>
            <WizardSection label="Exclusividade">{journey && <p className="j-calc-help">Tempo sem divulgar marcas concorrentes.</p>}<SegmentedChoice value={exclusivity} onChange={setExclusivity} options={[{ value: "nenhuma", label: "Sem" }, { value: "7d", label: "7 dias" }, { value: "15d", label: "15 dias" }, { value: "30d", label: "30 dias" }, { value: "90d", label: "90 dias" }]} /></WizardSection>
          </div> : null}

          {step === 2 ? <div className="grid gap-4">
            {!journey && <p className="text-[12px] leading-5 text-zinc-500">Considere a marca e como o conteúdo será distribuído.</p>}
            {(!journey || !isUgc) && <WizardSection label="Distribuição"><div className="divide-y divide-zinc-100"><BooleanChoice label="Collab no Instagram" value={instagramCollab} onChange={setInstagramCollab} /><BooleanChoice label="Repost no TikTok" value={repostTikTok} onChange={setRepostTikTok} />{!journey && <BooleanChoice label="UGC para perfil da marca" value={contentModel === "ugc_whitelabel"} onChange={(value) => setContentModel(value ? "ugc_whitelabel" : "publicidade_perfil")} />}</div></WizardSection>}
            <WizardSection label={journey ? "A marca e a parceria" : "Contexto"}><div className="grid gap-4"><div><p className="mb-2 text-[11px] font-medium text-zinc-600">Porte da marca</p>{journey && <details className="j-calc-tip"><summary aria-label="Entender esta escolha">?</summary><p>Considere a dimensão da empresa que está contratando.</p></details>}<SegmentedChoice value={brandSize} onChange={setBrandSize} options={[{ value: "pequena", label: "Pequena" }, { value: "media", label: "Média" }, { value: "grande", label: "Grande" }]} /></div><div><p className="mb-2 text-[11px] font-medium text-zinc-600">Risco de imagem</p>{journey && <details className="j-calc-tip"><summary aria-label="Entender esta escolha">?</summary><p>Quanto essa associação pode gerar exposição ou conflito com o que você representa.</p></details>}<SegmentedChoice value={imageRisk} onChange={setImageRisk} options={[{ value: "baixo", label: "Baixo" }, { value: "medio", label: "Médio" }, { value: "alto", label: "Alto" }]} /></div><div><p className="mb-2 text-[11px] font-medium text-zinc-600">Ganho estratégico</p>{journey && <details className="j-calc-tip"><summary aria-label="Entender esta escolha">?</summary><p>Quanto a parceria contribui para seu posicionamento e abre espaço para negociar.</p></details>}<SegmentedChoice value={strategicGain} onChange={setStrategicGain} options={[{ value: "baixo", label: "Baixo" }, { value: "medio", label: "Médio" }, { value: "alto", label: "Alto" }]} /></div></div></WizardSection>
          </div> : null}
          ...
          {step === 4 ? <div>
            {isSubmitting ? <div className="bg-zinc-950 px-5 py-6 text-white" role="status" aria-live="polite"><div className="flex items-center gap-3"><svg className="h-5 w-5 animate-spin text-white/60" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg><p className="text-sm font-semibold">Calculando sua faixa</p></div><p className="mt-3 text-sm leading-6 text-white/55">{isUgc ? "Combinando produção, uso pela marca e contexto." : "Combinando métricas, escopo, contexto e seu histórico."}</p></div> : error ? <div className="border border-red-100 bg-red-50 p-4" role="alert"><p className="text-sm font-semibold text-red-700">Não foi possível calcular</p><p className="mt-2 text-sm leading-6 text-red-600">{error}</p></div> : visibleResult ? <div>
              ...
              {journey && <div className="j-calc-receipt"><h3>Confira sua proposta</h3><small>Toque para ajustar</small>
                <button type="button" onClick={() => setStep(0)}><span><small>Entregas</small><strong>{buildDeliverablesLabel(quantities)}</strong></span><span>↗</span></button>
                <button type="button" onClick={() => setStep(1)}><span><small>Uso e exclusividade</small><strong>{labelForUsageRights(usageRights)} · {exclusivity === "nenhuma" ? "Sem exclusividade" : exclusivity}</strong></span><span>↗</span></button>
                <button type="button" onClick={() => setStep(2)}><span><small>A marca e a parceria</small><strong>Marca {brandSize} · risco {imageRisk} · ganho {strategicGain}</strong></span><span>↗</span></button>
                {!isUgc && <button type="button" onClick={() => setStep(3)}><span><small>Seu histórico</small><strong>{historyChoice === "use" ? "Usar meu valor habitual" : "Só a sugestão"}</strong></span><span>↗</span></button>}
              </div>}
