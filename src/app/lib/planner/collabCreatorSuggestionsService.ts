...
import {
  scoreCollabCreator,
  type CollabCreatorMatchType,
  type CollabCreatorScoreResult,
} from "@/app/lib/planner/collabCreatorScoring";
...
export type CollabCreatorSuggestion = {
  id: string;
  rank: number;
  name: string;
  username?: string | null;
  ...
  scoreParts: CollabCreatorScoreResult["scoreParts"];
};

export type CollabCreatorSuggestionCategories = {
  context?: unknown;
  proposal?: unknown;
  reference?: unknown;
};

export type BuildCollabCreatorSuggestionsInput = {
  viewerId: string;
  categories?: CollabCreatorSuggestionCategories;
...
export function normalizeCollabStringArray(value: unknown): string[] {
...
async function fetchThemeCreatorSuggestions(params: {
  ...
  const pipeline: PipelineStage[] = [
    ...
    { $addFields: { metricValue: "$avgInteractions" } },
...
export async function buildCollabCreatorSuggestions(
  input: BuildCollabCreatorSuggestionsInput,
): Promise<BuildCollabCreatorSuggestionsResult> {
  const viewerId = input.viewerId;
  const categories = input.categories || {};
  const contextIds = normalizeCollabStringArray(categories.context);
  const contextId = contextIds[0] || null;
  const contextLabel = contextId ? getCategoryByValue(contextId, "context")?.label || contextId : null;
  const themeKeyword =
    (typeof input.themeKeyword === "string" && input.themeKeyword.trim()) ||
    (typeof input.title === "string" && input.title.trim()) ||
    null;
  const periodDays = Math.max(30, Math.min(365, Number(input.periodDays) || 180));
