...
- `/dashboard` (destino do login), `/dashboard/profile` e `/dashboard/boards/mobile-strategic-profile` redirecionam para a Jornada via `buildJourneyHref` (`src/app/dashboard/jornada/journeyRoute.ts`), **preservando a query**: checkout, `instagramLinked`, `activation`, `action`, `source=chatgpt`. `tab=collabs` vira `view=collabs`; `openCommunity=1` vira `view=comunidade`. Subpáginas (`.../mobile-strategic-profile/settings/...`) seguem vivas. `board=post-creation` continua indo para `/calendar`.
- Middleware manda o celular de `/dashboard` e `/dashboard/home` direto para a Jornada.
- **A Home antiga do desktop ficou sem entrada — decisão do Arthur (15/09): unificar celular e computador na Jornada, sem manter a Home como alternativa** (duas experiências vivas = duas para manter). `next.config.js` já redirecionava `/dashboard/home` → `/dashboard`; se um dia precisar dela de volta, é esse redirect que tem de sair.
- Sem sessão, o layout do `/dashboard` manda para `/login?callbackUrl=...` antes da página rodar — o redirecionamento das páginas só se confere logado.
...
- Tom sobre tom (15/09, referências MIS e BrandLovrs): página `--j-page` **branca**, card `--j-card` cinza neutro (`#f5f5f4` no CSS publicado; esta nota dizia `#fafafa`) sem contorno (areia/bege foi achado pesado; o cinza veio de uma referência do Arthur), e o card é uma peça só — nada de quadro de outra cor dentro dele (a narrativa no Perfil em quadro próprio leu como "card dentro de card" e foi rejeitada). Etiquetas dentro do card (territórios, prazo, formatos) = fundo do card + contorno escuro `--j-outline`. Fundo de página colorido também foi testado e rejeitado. A D2C ainda não tem cor de marca (é preto e branco) — o rosa do `design-system/tokens.css` não é marca. Preto é o único contraste forte: ações e a pílula do dado que decide (`--j-highlight`, onde a futura cor entra trocando uma linha). Cor só em status, pequena (bolinha + texto). **Descartado:** tons de categoria (rosé narrativa/mídia kit, lilás ferramentas/Claude, sálvia parcerias) — cinco famílias na mesma tela recriaram o embaralhado que se queria resolver. Mídia kit fica branco com contorno (vitrine para marca). O card de Collabs lê `--j-card` com fallback, então o app antigo não muda.
