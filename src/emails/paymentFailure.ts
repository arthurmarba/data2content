...
export function paymentFailureEmail({
  ...
  const subject = "Falha no pagamento da sua assinatura do Plano Pro";

  const text = [
    `${greeting}`,
    "",
    `Não conseguimos processar o pagamento de ${formattedAmount} referente à sua assinatura do Plano Pro.`,
  ...
  const html = `
    <p>${greeting}</p>
    <p>Não conseguimos processar o pagamento de <strong>${formattedAmount}</strong> referente à sua assinatura do Plano Pro.</p>
    ${
      invoiceLink
        ? `<p>${invoiceLink}</p>`
        : ""
