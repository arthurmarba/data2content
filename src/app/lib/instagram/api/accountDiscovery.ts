...
// Tipo local para dados de página
 ...
 * Utiliza o LLAT do usuário para listar páginas via /me/accounts.
 *
...
export async function fetchAvailableInstagramAccounts(
  ...
  const TAG = '[fetchAvailableInstagramAccounts v2.1_user_token_only]'; 
  ...
  try {
    ...
    if (llatError.message && (llatError.message.toLowerCase().includes('token') || isTokenInvalidError(undefined, undefined, llatError.message))) {
      ...
      // Convertendo string para ObjectId se necessário
  ...
  // Etapa 2: Listar Páginas e Contas IG usando o LLAT do usuário
  try {
    let allPagesData: PageAccountData[] = [];
    let fetchError: Error | null = null;
    const fetchLogContext = `${TAG} (User LLAT Flow)`;
    
    logger.info(`${fetchLogContext} Utilizando User LLAT e /me/accounts para listar contas para o usuário ${userId}.`);
    if (!userLongLivedAccessToken) {
      // Esta verificação é uma segurança adicional, mas o bloco catch anterior já deveria ter tratado isso.
    ...
    logger.info(`${fetchLogContext} Busca paginada de contas FB concluída para User ${userId}. ${allPagesData.length} Páginas FB encontradas.`);
    ...
    if (availableIgAccounts.length === 0) {
      const errorMsg = "Nenhuma conta profissional do Instagram (Comercial ou Criador de Conteúdo) foi encontrada vinculada às Páginas do Facebook que você gerencia.";
