...
export function paymentReceiptEmail({
  ...
  const subject = `Recibo da assinatura PRO ${invoiceNumber ? `(${invoiceNumber})` : ""}`.trim();

  const textParts = [
    `${greeting}`,
    "",
    `Registramos o pagamento de ${formattedAmount} referente à assinatura PRO (${invoiceLabel}).`,
  ];
  if (periodLabel) {
  ...
  textParts.push(
    ...
    "",
    "Obrigado por manter o PRO ativo! Se precisar de suporte, é só responder este e-mail.",
    "Equipe Data2Content"
  );
  ...
  const html = `
    <p>${greeting}</p>
    <p>Registramos o pagamento de <strong>${formattedAmount}</strong> referente à assinatura PRO (${invoiceLabel}).</p>
    ${
      periodLabel
    ...
    <p style="margin-top:20px;">Obrigado por manter o PRO ativo! Se precisar de suporte, é só responder este e-mail.<br/>Equipe Data2Content</p>
  `;
