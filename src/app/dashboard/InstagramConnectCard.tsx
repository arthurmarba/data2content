...
const InstagramConnectCard: React.FC<InstagramConnectCardProps> = ({
  ...
  // Determina se o Instagram está efetivamente conectado E o usuário tem acesso
  ...
  useEffect(() => {
    ...
    if (urlParams.get('instagramLinked') === 'true' || urlParams.get('instagramDisconnected') === 'true') {
      logger.info("Parâmetro de URL detectado. Forçando atualização da sessão.");
      update(); 
  ...
  const handleInitiateFacebookLink = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (user?.igConnectionError || user?.availableIgAccounts) {
      logger.debug("Limpando erros da sessão (igConnectionError, availableIgAccounts) localmente antes de iniciar vinculação FB.");
      await update({
          ...session,
          user: {
              ...user,
    ...
    try {
      logger.info("Assinante: Chamando /api/auth/iniciar-vinculacao-fb");
      ...
      if (!res.ok) {
        ...
        logger.error("Assinante: Erro ao chamar /api/auth/iniciar-vinculacao-fb:", errData);
        throw new Error(errData.message || 'Falha ao preparar vinculação com Facebook.');
      }
      logger.info("Assinante: /api/auth/iniciar-vinculacao-fb OK. Iniciando signIn('facebook').");
      signIn('facebook', { callbackUrl: '/dashboard?instagramLinked=true' });
    } catch (e: any) {
      logger.error('Assinante: Erro ao iniciar vinculação:', e);
  ...
  const handleDisconnectInstagram = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      logger.info("Assinante: Chamando /api/instagram/disconnect");
      ...
      if (!res.ok) {
        ...
        logger.error("Assinante: Erro ao chamar /api/instagram/disconnect:", errData);
        throw new Error(errData.message || 'Falha ao desconectar Instagram.');
      }
      logger.info("Assinante: /api/instagram/disconnect OK. Forçando atualização da sessão.");
      await update(); 
      setShowSuccessToastFromCard("Instagram desconectado com sucesso!");
    } catch (e: any) {
      logger.error('Assinante: Erro ao desconectar:', e);
            ...
            {isEffectivelyInstagramConnected ? (
              ...
              <div className="flex flex-col items-center sm:items-end w-full sm:w-auto">
                <button
                  ...
                  disabled={canAccessFeatures && (isLinking || (isLoadingSession && !session))}
                  className={`w-full sm:w-auto px-6 py-3 text-sm font-medium rounded-lg flex items-center justify-center gap-2.5 
                              ...
                              ${(canAccessFeatures && (isLinking || (isLoadingSession && !session))) ? 'opacity-70 cursor-wait' : ''}
                              ${mainButtonStyles} 
                            `}
