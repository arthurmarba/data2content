...
export async function executeVideoAnalysis(request: Request, session: MobileStrategicProfileRealAnalysisSession, execution: { requestId?: string; resumeMedia?: import("@/app/dashboard/boards/videoUpload/videoNarrativeMediaProbe").VideoNarrativeVerifiedMediaMetadata; runProvider?: import("@/app/dashboard/boards/videoUpload/videoNarrativeRealAnalysisOrchestrator").VideoNarrativeRealAnalysisOrchestratorDeps["runProvider"] } = {}) {
  const requestId = execution.requestId ?? `msp-real-${randomUUID()}`;
  try {
    const featureFlags = {
    ...
    const result = await runVideoNarrativeRealAnalysisOrchestrator({
      ...
      deps: {
        ...
        ...(execution.runProvider ? { runProvider: execution.runProvider, resumeMedia: execution.resumeMedia } : {}),
        cleanupTemporaryUpload: async ({ uploadSessionId, objectKey, reason }) => {
          if (execution.runProvider) return; // O trabalho persistido coordena retenção e limpeza.
