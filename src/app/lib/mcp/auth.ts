...
import {
  getMcpAdminConnectionScopes,
  getMcpAdminRequiredScope,
  getMcpAdminResourceMetadataUrl,
...
function readOAuthConfig(audienceOverride?: string): McpOAuthConfig {
  let issuer: string;
  let audience: string;
  let jwksUrl: string;
  try {
    issuer = getMcpOAuthIssuer();
    audience = audienceOverride || getMcpOAuthAudience();
...
function maybeResolveDevelopmentIdentity(
  request: Request,
  scopes = getMcpConnectionScopes(),
): McpAuthenticatedIdentity | null {
...
async function authenticateMcpRequestForPolicy(
  request: Request,
  policy: { audience: string; requiredScopes: string[]; developmentScopes?: string[] },
): Promise<McpAuthenticatedIdentity> {
  const developmentIdentity = maybeResolveDevelopmentIdentity(
    request,
    policy.developmentScopes ?? policy.requiredScopes,
  );
  if (developmentIdentity) return developmentIdentity;

  const token = parseBearerToken(request);
  const config = readOAuthConfig(policy.audience);
  ...
  try {
    ...
    if (missingScopes.length) {
      throw new McpAuthError(
        "insufficient_scope",
        403,
        `A conexão precisa ser reautorizada com os scopes: ${missingScopes.join(", ")}.`,
      );
    }
...
export async function authenticateAdminMcpRequest(request: Request): Promise<McpAuthenticatedIdentity> {
  return authenticateMcpRequestForPolicy(request, {
    audience: getMcpAdminServerUrl(),
    requiredScopes: [getMcpAdminRequiredScope()],
    developmentScopes: getMcpAdminConnectionScopes(),
  });
}
...
export function buildMcpWwwAuthenticateHeader(error?: McpAuthError): string {
  const parts = [
    ...
    `scope="${getMcpConnectionScopes().join(" ")}"`,
  ];
...
export function buildMcpAdminWwwAuthenticateHeader(error?: McpAuthError): string {
  const parts = [
    `Bearer resource_metadata="${getMcpAdminResourceMetadataUrl()}"`,
    `scope="${getMcpAdminConnectionScopes().join(" ")}"`,
  ];
  if (error && error.code !== "auth_not_configured") {
    parts.push(`error="${error.code === "missing_token" ? "invalid_token" : error.code}"`);
  }
