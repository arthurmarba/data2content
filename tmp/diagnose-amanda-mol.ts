...
async function main() {
  ...
  const users: any[] = await User.find({
    $or: [
      { name: /amanda.*mol|mol.*amanda/i },
      { username: /amanda.*mol|mol.*amanda/i },
      { email: /amanda.*mol|mol.*amanda/i },
    ],
  })
  ...
  console.log(`Amanda Mol: ${users.length} candidato(s)`);
  ...
  for (const u of users) {
    ...
    const post = (p: any) => ({
      ...
      description: p.description?.replace(/\s+/g, " ").slice(0, 360) ?? null,
      proposal: p.proposal ?? [],
      context: p.context ?? [],
      tone: p.tone ?? [],
      stats: {
